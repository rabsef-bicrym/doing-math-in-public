...
// Support of the regular octagon. The argument is normalized by pi. Using
// exact dyadic sector boundaries avoids ambiguity caused by an enclosure of pi.
...
// Piecewise lower bounds tied to kernel-checked analytic theorems:
// [0,1/4] antitonicity; endpoint monotonicity and the exact 4/625 bound on
// the first large sector; the (3/5)*beta bound on the second sector; and the
// d^2 bound on the third. Full directed interval evaluation is used whenever
// an endpoint theorem does not apply. Global nonnegativity permits clamping a
// dependency-widened lower endpoint to zero.
...
static double delta_lower_box(const I& U) {
  ...
  if (lo < 0.25) {
    const double b = std::min(hi, 0.25);
    add_bound(delta_point_lower_small(b));
  }
...
static void emit_tree(std::ofstream& out, const Box& b, int nA, int nB,
  ...
  if (!e.feasible) {
    put8(out, 7);
    ++st.leaves; ++st.infeasible;
    return;
  }
  if (e.lb > 0.0) {
    put8(out, 6);
    put64(out, 1);
    ++st.leaves; ++st.positive;
    st.worst = std::min(st.worst, e.lb);
    return;
  }

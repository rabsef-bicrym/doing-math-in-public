...
The isoperimetrix is normalized to have apothem one.  Its circumradius is

  `R = sqrt (4 - 2 sqrt 2)`.

This module proves the two identities used by the sector bridge:

  `R cos(π/8) = 1`,
  `R sin(π/8) = sqrt 2 - 1`.

The proof uses only double-angle formulas, positivity, and exact square-root
algebra; no prepackaged eighth-angle value is assumed.
-/

namespace OctagonSupportNormalization
...
lemma eighth_le_half_pi : Real.pi / 8 ≤ Real.pi / 2 := by
  nlinarith [Real.pi_pos]
...
/-- The corresponding tangent ratio is `sqrt 2 - 1`. -/
...
end OctagonSupportNormalization
